...
export default function Home() {
  ...
  const fetchProperties = useCallback(async (filters?: any) => {
    try {
      setLoading(true);
      // Convert filters to the format expected by API
      const apiFilters = filters || currentFilters;
      const data = await api.getProperties({
        location: apiFilters.location,
        minPrice: apiFilters.minPrice,
        maxPrice: apiFilters.maxPrice,
        checkIn: apiFilters.checkIn?.toISOString(),
        checkOut: apiFilters.checkOut?.toISOString(),
        homeType: apiFilters.homeType,
        amenities: apiFilters.amenities,
        bedrooms: apiFilters.bedrooms,
      });
      setProperties(data);
      setError(null);
    } catch (err) {
  ...
  const handleSearch = useCallback((filters: any) => {
    ...
    if (filters.maxPrice && filters.maxPrice < 5000) params.set('maxPrice', filters.maxPrice.toString());
          ...
          <>
            <div className="mb-4 text-sm text-gray-600 mt-8">
              ...
              {currentFilters.checkIn && currentFilters.checkOut && (
                ` from ${currentFilters.checkIn.toLocaleDateString()} to ${currentFilters.checkOut.toLocaleDateString()}`
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
